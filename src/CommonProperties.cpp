...
LRESULT CommonProperties::OnListViewGetInfoTipNotification(int /*controlID*/, LPNMHDR pNotificationDetails, BOOL& /*wasHandled*/)
{
	...
	switch(pDetails->iItem) {
		...
		case 3:
			...
			break;
		case 4:
			ATLVERIFY(SUCCEEDED(StringCchCopy(pBuffer, pDetails->cchTextMax + 1, TEXT("GetInfoTipText"))));
			break;
	}
...
LRESULT CommonProperties::OnLoadSettingsFromFile(WORD /*notifyCode*/, WORD /*controlID*/, HWND /*hWnd*/, BOOL& /*wasHandled*/)
{
	...
	CComQIPtr<ITrackBar, &IID_ITrackBar> pControl(m_ppUnk[0]);
	if(pControl) {
...
LRESULT CommonProperties::OnSaveSettingsToFile(WORD /*notifyCode*/, WORD /*controlID*/, HWND /*hWnd*/, BOOL& /*wasHandled*/)
{
	...
	CComQIPtr<ITrackBar, &IID_ITrackBar> pControl(m_ppUnk[0]);
	if(pControl) {
		CFileDialog dlg(FALSE, NULL, TEXT("TrackBar Settings.dat"), OFN_ENABLESIZING | OFN_EXPLORER | OFN_CREATEPROMPT | OFN_HIDEREADONLY | OFN_LONGNAMES | OFN_PATHMUSTEXIST | OFN_OVERWRITEPROMPT, TEXT("All files\0*.*\0\0"), *this);
		if(dlg.DoModal() == IDOK) {
			CComBSTR file = dlg.m_szFileName;
...
void CommonProperties::ApplySettings(void)
{
	for(UINT object = 0; object < m_nObjects; ++object) {
		CComQIPtr<ITrackBar, &IID_ITrackBar> pControl(m_ppUnk[object]);
		if(pControl) {
			...
			SetBit(controls.disabledEventsList.GetItemState(4, LVIS_STATEIMAGEMASK), l, deGetInfoTipText);
...
void CommonProperties::LoadSettings(void)
{
	...
	if(pDisabledEvents) {
		...
		controls.disabledEventsList.AddItem(4, 0, TEXT("GetInfoTipText event"));
		controls.disabledEventsList.SetItemState(4, CalculateStateImageMask(m_nObjects, pl, deGetInfoTipText), LVIS_STATEIMAGEMASK);
